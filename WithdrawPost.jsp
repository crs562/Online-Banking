...
<% ToAccount = ""; %>
<% FromAccount = request.getParameter("SourceAccount"); %>
		...
		if (FromAccount.equals("Checking..."+ AccountNumber.substring(4, 8))){
			...
			if(CA.withdraw()) {
				...
				if(TZ.recordTransaction())
					out.println("Withdraw Transaction successful in Checking!");
				else
					out.println("Withdraw Transaction Failed in Checking!");
			} else
				out.println("Withdraw failed in Checking.");
			...
			if(SA.withdraw()) {
				Transaction TZ = new Transaction(ToAccount, FromAccount, Name, UName, Balance);
				if(TZ.recordTransaction())
					out.println("Withdraw Transaction successful in Saving!");
				else
					out.println("Withdraw Transaction Failed in Saving!");
			} else
				out.println("Withdraw failed in Saving.");
		} else
			out.println("Please Choose an Account Type!");
	}
%>