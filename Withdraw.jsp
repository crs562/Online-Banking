...
<HTML>
	<HEAD>
		<title> Withdraw Money | MoneyExchange </title>
	</HEAD>
			
	<script type = "text/javascript">
		document.WithdrawForm.UserID.focus();
		function checkInputs() {
			var Prompts = "";
			UserID = window.document.WithdrawForm.UserID.value;
			FromAccount = window.document.WithdrawForm.FromAccount.value;
			WithdrawAmount = window.document.WithdrawForm.MoneyField.value;
			if (UserID == "" || FromAccount == "" || WithdrawAmount == "") {
				if (UserID == "")
				     	Prompts +="Please enter your userID!\n";
				if (FromAccount == "")
				     	Prompts +="Please select your Account Type!\n";
				if (WithdrawAmount == "")
				        Prompts +="Please enter your Withdraw Amount!\n";
				if (Prompts != "")
	...
	<BODY>
		...
		<FORM NAME="WithdrawForm" action = "WithdrawPost.jsp" method = "GET">
			<INPUT TYPE='hidden' NAME='UserID' VALUE='<%=request.getParameter("UserID")%>'>
			<INPUT TYPE='hidden' NAME='CustName' VALUE='<%=request.getParameter("CustName")%>'>
				<TABLE cellPadding=3 ALIGN='center'>
					<tr> <td> <h2> <% out.println("Withdraw Money | MoneyExchange"); %> </h2> </td> </tr>
					<TR bgcolor='#ECFAEB'>
						<TD>From Account:</TD>
        		        		<TD>
				...
				<CENTER><INPUT TYPE="SUBMIT" NAME='submitBNTN' VALUE='Withdraw' onClick="checkInputs()"></CENTER><BR>
			</FORM>
		</BODY>
	</HTML>